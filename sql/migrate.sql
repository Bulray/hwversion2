...
SELECT DISTINCT outcome_subtype FROM animals WHERE IS outcome_subtype IS NOT NULL;

INSERT INTO outcome_type (name)
SELECT DISTINCT outcome_type FROM animals WHERE IS outcome_type IS NOT NULL;
...
LEFT JOIN animal_color as animal_color1
    ON animal_color1.name = animals.color1_id
LEFT JOIN animal_color as animal_color2
    ON animal_color2.name = animals.color2_id
LEFT JOIN outcome_subtype
    ON outcome_subtype.name = animal.outcome_subtype
LEFT JOIN
